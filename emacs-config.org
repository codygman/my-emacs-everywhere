...
*** what is listed is not necessarily what I do in practice
*** instead I've listed the best ways I know of doing a common task with my current config or
*** additionally each section has a current and future changes section to constantly improve workflow(s)
** org mode
*** current
**** general usage
***** Open daily agenda for today with =SPC o d=
***** Capture from anywhere with =SPC c c= and do so using =helm-org-capture-templates=
***** finding any random org heading: =SPC o r= (org rifle)
***** starting on a task not in agenda: open an org file with =C-c [= if none opened and =SPC o r C-z <ind> SPC 2 , c i , t t i=
***** starting on a task in agenda: navigate to, =RET , c i , t t i SPC u , s b SPC 1 SPC w m=
**** navigating org headings
***** go to parent: =g h=
***** get context "why am I doing this?": =helm-org-parent-headings=
***** go up heading: ={=
***** go down heading: =}=
***** go to parent heading: =}=
***** go to subheading(expanded): =j=
***** go to subheading(collapsed): =TAB j=
**** agenda
***** rescheduling headings preserving time:  B D
****** see [[*install and configure org][install and configure org]]
***** clock in:  =i=
***** clock out: =o=
***** clock cancel: =x=
***** schedule:  =s=
...
***** logging mode: =l=
***** set priority: =,=
***** kill agenda item: =C-k=
*** future changes
**** no general org mode changes proposed yet
**** general usage
***** TODO start task not in agenda: =SPC o r C-z <start>=
***** TODO start task in agenda: maybe =gi= (clocks in, marks in progress, opens in indirect buffer, maximizes
**** agenda
***** probably defaults of evil-collection
** working in projects
*** current
**** navigation
***** prefer finding things via filenames with =SPC p p=
***** prefer finding things with =SPC f f C-z <git> RET <query> RET= :bad:timeSink:
***** go to definition
****** uses evil-goto-definition
***** when in sourcetree with duplicate filenames, knowing which one you are at consists of =SPC f f= and seeing current directory
***** find file in another branch: =SPC SPC <mff>= for =magit-find-file=
**** magit
***** stage hunks of file: =SPC g s= then =s= on hunks you want staged
***** stage all changes in a file: =SPC g s ] ] s=
***** commit and stage all changes done in repo: =SPC g s c c y <commit message> C-c C-c=
*** future changes
**** navigation
***** [[file:~/org/org.org::*prefer finding things via rg for project with =SPC p g= set to =projectile-rg=][prefer finding things via rg for project with =SPC p g= set to =projectile-rg=]]
***** TODO go to definition preferring smarter jumps first
****** goal: Try to use lsp go-to-definition to get exactly where you want and fall-through to less precise methods quickly
****** goal: this should all happen in less than 1 second every time even if just falling back to ripgrep
****** TODO should use smart-jump and prefer lsp-* engines for 500ms
****** TODO next it should prefer tag sources if they exist
****** TODO finally it should use dumb-jump configured to *only use ripgrep*
***** TODO default persistent action of =SPC p p= should be =magit-status= instead of =projectile-find-files=
****** alternatively projectile-find-files could become smarter and add entries for recently changed git files
******* clicking on one of those that doesn't exist in current working tree would switch branches with magit
******* these entries would also need the current branch
***** TODO when in sourcetree with duplicate filenames, knowing which one you are at consists of looking at the buffer that contains parent directory name
**** magit
***** TODO stage all changes in a file: =, g f s=
***** TODO commit and stage all changes done in repo: =, g S c c y <commit message> , ,= (not sure about this one)
** literate programming
*** current
**** yasnippet is used for common tasks like (note: roughly ordered by use)
***** TODO sh
#+begin_src org
...
#+begin_src org
  ,#+begin_example
  << contents of copy buffer >>
  ,#+end_example
#+end_src
**** a custom ob-sh-evaluate is used to allow logging long running processes like servers output to a file without blocking emacs
*** future changes
**** yasnippet is used for common tasks like (note: roughly ordered by use)
***** sh
****** snippet expands to
#+begin_src org
  ...
  ,#+end_src
#+end_src
****** pretty sure I want to remove verbatim so I can use named source blocks more
****** at least if I figure out how to streamline the process better
...
#+begin_src emacs-lisp
    (use-package haskell-mode
      :if (string-equal (getenv "EMACSFOR") "WORK")
      :defer t
      :mode "\\.hs\\'"
      :commands haskell-mode
      :init
      (message "work haskell-mode loading")
      (setq haskell-stylish-on-save nil)
      (setq haskell-mode-stylish-haskell-path "brittany")
      (setq haskell-mode-stylish-haskell-arguments "--config-file /Users/codygman/ITProTV/smurf/nucleus/brittany.yaml")
...
*** lsp-haskell
#+begin_src emacs-lisp
  (when (string-equal (getenv "EMACSFOR") "WORK")
  (use-package lsp-haskell
    :init
    (setq lsp-haskell-process-path-hie "/Users/codygman/.local/bin/hie-wrapper"))
  )
...
#+end_src
*** haskell-mode eval after config
#+begin_src emacs-lisp :eval no
  (when (string-equal (getenv "EMACSFOR") "WORK")

    (setq haskell-process-args-stack-ghci '("--ghci-options=-ferror-spans -w"))
...
#+end_src
* archive of commented out code
#+begin_src emacs-lisp


  ;; TODO is it necessary to do a straight-use-package followed by a use-package block to configure?




  ;; give us more room with helm buffer names
  ;; NOTE just leaving at default of 60 so that helm works same even when not fullscreen
  ;; you can just press C-[ in a helm buffer to show full filenames
  ...
  ;; desktop+ that works with indirect buffers
  ;; (straight-use-package
  ;;  '(desktop+ :type git :host github :repo "ffevotte/desktop-plus"))

  ;; ;; ;; persp-mode (like winum but w file persistence)
  ;; (straight-use-package
  ;;  '(el-patch :type git :host github :repo "Bad-ptr/persp-mode.el"))

  ;; (add-hook 'after-init-hook #'(lambda () (persp-mode 1)))
  ;; make indirect buffers work with persp-mode
  ...
  ;; weird error
  ;; Error (use-package): org-pomodoro/:catch: Invalid read syntax: ")"
  ...
  ;; hare readme recommends this
  ;; (add-to-list 'load-path "~/.emacs.d/HaRe/elisp")
  ;; (require 'hare)
  ...
  ;; WORKING haskell-mode stuff - 2018-11-20
  ...
  ;; (org-agenda nil "c")
  ;; (progn (switch-to-buffer "*Org Agenda*") (delete-other-windows))
  ;; (add-to-list 'initial-frame-alist '(fullscreen . maximized)) ;; only maximized... but only reliable way I could get it to do it

  ;; custom function definitions I may need to move to top
  ...
  ;; 		 org-clock-history)
  ;; 	(insert
  ;; 	 (replace-regexp-in-string "/home/cody/" "~/" (format "(setq org-clock-stored-history '(%s))\n"
  ;; 							      (mapconcat
  ;; 							       (lambda (m)
  ...
  ;; ~/.emacs.d/private/custom.el contains customizations per machine/for work/play/personal based on machine
  ;; examples
  ...
  ;; todo refactor this shouldn't be necessary here
